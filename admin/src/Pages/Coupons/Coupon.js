...
const Coupon = () => {
  ...
  const createAndEditCoupon = async (type) => {
    try {
      setLoading(true);
      if (type === 'edit') {
        await axios.post(`http://localhost:7400/api/v1/update-coupon/${selectedCoupon?.code}`, formData);
      } else {
        await axios.post('http://localhost:7400/api/v1/add-coupon', formData);
      }
  ...
  const deleteCoupon = async (code) => {
    try {
      setLoading(true);
      await axios.delete(`http://localhost:7400/api/v1/delete-coupon/${code}`);
